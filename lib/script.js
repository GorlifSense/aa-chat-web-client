...
//FIXME should be correct npm module
...
(function () {
  ...
  this.addMessage = function addMessage(id, func) {
    ...
    if (_.isEmpty(value)) {
      ...
      input.value = '';
      if (func) {
        func(messages, messages.list[id]);
      }
    }
  };
  chat.html = {};
  ...
  chat.html.functions = {
    ...
    'add-user': function (id, nickname) {
      this.controller.addUser(id, nickname, function (global, local) {
        chat.html.apply(this, {
          'className': 'message-' + local['id'],
          'innerHTML': local['message'],
          'tag': 'div'
        });
      });
    },
